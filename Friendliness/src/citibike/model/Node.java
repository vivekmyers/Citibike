...
class Node implements Iterable<Road> {
	...
	public String toString() {
		return id + " -> [" + next.stream().map(Road::getDestination).map(Node::getId)
				.collect(joining(", ")) + ']';
	}
	...
	public void connect(Node v, double dist) {
		if (v == this)
			return;
        Road r = new Road(dist, v);
		if (!next.contains(r))
			next.add(r);
	}
	
	public void bconnect(Node v, double w, double dist) {
		if (v == this)
			return;
        Road r = new BikeLane(dist, v, w);
		if (!next.contains(r))
			next.add(r);
	}
}
