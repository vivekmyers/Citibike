...
public class Distribution {
	...
	public static void generate(List<String> list) throws IOException, URISyntaxException {
		// System.out.println(list);
		for (int i = 0; i < 51; i++)
			System.out.print("—");
		System.out.println("\nReading Data");
		BufferedReader br = Files.newBufferedReader(
				Paths.get(Distribution.class.getResource("usagedata.txt").toURI()));
		...
		br.lines().unordered().parallel().forEach(s -> {
			try {
				...
				if (!list.contains("" + k1) || !list.contains("" + k2))
					return;
				if (k1 > k2) {
					int tmp = k1;
					k1 = k2;
